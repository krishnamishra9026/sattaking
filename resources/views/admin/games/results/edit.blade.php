...
<div class="card">
    <div class="card-header">
        <h3 class="card-title">Edit Game Result</h3>
        <div class="card-toolbar">
            <a href="{{ route('admin.games.result.index') }}" class="btn btn-light">Back</a>
        </div>
    </div>
    <div class="card-body">
        <form action="{{ route('admin.games.result.update', $result->id) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="mb-3">
                ...
                <select name="game_id" id="game_id" class="form-select @error('game_id') is-invalid @enderror">
                    ...
                    @foreach($games as $game)
                        <option value="{{ $game->id }}" {{ old('game_id', $result->game_id) == $game->id ? 'selected' : '' }}>
                            {{ $game->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-3">
                ...
                <input type="text" name="result_number" id="result_number" 
                    ...
                    value="{{ old('result_number', $result->result_number) }}">
                @error('result_number')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
